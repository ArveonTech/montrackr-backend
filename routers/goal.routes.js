...
goalsRoute.patch(``, verifyToken, verifyUser, verifyOwnership, verifyGoalExist, async (req, res, next) => {
  try {
    ...
    next(new GoalError(`Error update goal user: ${error.message}`, 400));
  }
});

// delete goal
goalsRoute.delete(``, verifyToken, verifyUser, verifyGoalExist, async (req, res, next) => {
  try {
    const { dataUserDB } = req;
    ...
    next(new GoalError(`Error delete goal: ${error.message}`, 400));
  }
});
